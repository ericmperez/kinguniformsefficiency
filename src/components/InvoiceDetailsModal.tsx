...
import {
  formatDateSpanish,
  formatDateOnlySpanish,
} from "../utils/dateFormatter";
import { useCartEditor } from "./CartEditHandler";
import { DeleteConfirmationModal } from "./DeleteConfirmationModal";
import {
  transformClientNameForDisplay,
  shouldAlwaysShowQuantities,
  isOncologicoClient,
  isChildrensHospitalClient,
  isExcludedFromQuantities,
} from "../utils/clientNameUtils";
...
const InvoiceDetailsModal: React.FC<InvoiceDetailsModalProps> = ({
  ...
  const [showQuantitiesForThisInvoice, setShowQuantitiesForThisInvoice] =
    React.useState(true);
  ...
  const [showSpecialServiceConfirmation, setShowSpecialServiceConfirmation] =
    React.useState(false);
  const [pendingSpecialServiceState, setPendingSpecialServiceState] =
  ...
  const markCartAsPrinted = async (cartId: string) => {
    ...
    try {
      ...
      console.log("✅ Cart marked as printed successfully:", {
        cartId,
        user: currentUser,
        timestamp: now,
      });
  ...
  const markCartAsModified = async (cartId: string) => {
    ...
    try {
      await onUpdateInvoice(localInvoice.id, { carts: updatedCarts });
      console.log("✅ Cart marked as modified successfully:", {
        cartId,
        user: currentUser,
        timestamp: now,
      });
  ...
  React.useEffect(() => {
    const currentCartData = localCarts
      .map((c) => `${c.id}:${c.name}`)
      .sort()
      .join("|");
    const newCartData = (localInvoice.carts || [])
      .map((c) => `${c.id}:${c.name}`)
      .sort()
      .join("|");
    ...
    if (currentCartData !== newCartData) {
      console.log(
        "🔄 Syncing localCarts with localInvoice.carts - changes detected:",
        {
          invoiceId: localInvoice.id,
          fromInvoice: localInvoice.carts?.map((c) => ({
            id: c.id,
            name: c.name,
          })),
      ...
      const newCarts = localInvoice.carts
        ? localInvoice.carts.map((cart) => ({
            ...cart,
      ...
      console.log(
        "✅ Updated localCarts:",
        newCarts.map((c) => ({ id: c.id, name: c.name }))
      );
    } else {
  ...
  const printAllCarts = () => {
    ...
    const printConfig =
      client?.printConfig?.cartPrintSettings || defaultCartPrintSettings;
    ...
    const getClientNameFontSize = () => {
      switch (printConfig.clientNameFontSize) {
        ...
        case "large":
          return "45px";
        default:
          return "35px"; // fallback to medium
      }
    };
    ...
    const generateAllCartsContent = () => {
      return localCarts
        .map((cart, index) => {
          const cartIndex = localInvoice.carts.findIndex(
            (c) => c.id === cart.id
          );
          ...
          <div style="page-break-after: ${
            index < localCarts.length - 1 ? "always" : "auto"
          };">
            <div style="
              width: 8in;
              height: 5in;
              ...
              ">
                ...
                ${
                  ...
                  <div style="
                    ...
                    border-radius: 6px;
                  ">
                    DELIVERY DATE: ${formatDateOnlySpanish(
                      localInvoice.deliveryDate
                    )}
                  </div>
                `
    ...
    setTimeout(async () => {
      printWindow.document.write(`
        <html>
          <head>
            <title>Print All Carts - ${transformClientNameForDisplay(
  ...
  const [showCreateCartConfirm, setShowCreateCartConfirm] =
    React.useState(false);
  ...
  return (
    <>
      ...
      >
                ...
                <div className="d-flex align-items-center me-3">
                  <div className="form-check mb-0">
                    <input
                      ...
                      onChange={(e) =>
                        handleSpecialServiceToggle(e.target.checked)
                      }
                      style={{
                        transform: "scale(1.3)",
                        accentColor: "#0E62A0",
                ...
                <div className="d-flex align-items-center justify-content-center">
                  ...
                  {editingDeliveryDate ? (
                    ...
                    <div className="d-flex align-items-center">
                      ...
                      >
                        {invoice.deliveryDate
                          ? formatDateSpanish(invoice.deliveryDate)
                          : "Not set"}
                      </h2>
                      <button
                  ...
                  <>
                    ...
                    {localCarts.length > 0 && (
                      <div className="ms-auto">
                        {(() => {
                          ...
                          const printedCarts = localCarts.filter(
                            (c) =>
                              c.lastPrintedAt &&
                              !c.needsReprint &&
                              (!c.lastModifiedAt ||
                                !c.lastPrintedAt ||
                                new Date(c.lastModifiedAt) <=
                                  new Date(c.lastPrintedAt))
                          ).length;
                    ...
                    {localInvoice.printHistory?.lastPrintAllCarts && (
                      <div className="col-12 mt-3">
                        ...
                        >
                          <i
                            className="bi bi-printer-fill me-2"
                            style={{ fontSize: "16px" }}
                          ></i>
                          <div>
              ...
              {localCarts.map((cart, cartIndex) => (
                ...
                >
                  ...
                  <div className="text-center mb-3">
                    <h3 className="enhanced-cart-name">{cart.name}</h3>
                  </div>
                        ...
                        return (
                          <div className="d-flex flex-column align-items-end">
                            ...
                            {cart.lastPrintedAt && cart.lastPrintedBy && (
                              ...
                              >
                                Last printed by:{" "}
                                <strong>{cart.lastPrintedBy}</strong>
                                <br />
                    ...
                    <div className="d-flex gap-2">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          const otherCarts = localInvoice.carts.filter(
                            (c) => c.id !== cart.id
                          );
                        ...
                        onClick={async () => {
                          if (window.confirm(`Delete cart '${cart.name}'?`)) {
                            try {
                              ...
                              console.log(
                                "🎉 Cart deletion completed successfully with direct approach"
                              );
                            } catch (error: any) {
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          if (
                            ...
                            console.log(
                              "🎯 Cart editing started with direct approach:",
                              {
                                cartId: cart.id,
                                oldName: cart.name,
                            ...
                            try {
                              // Use the new direct cart editor
                              await cartEditor.updateCartName(
                                cart.id,
                                newName.trim()
                              );
                              ...
                              console.log(
                                "🎉 Cart name update completed successfully with direct approach"
                              );
                            } catch (error: any) {
                              console.error(
            ...
            onConfirm={async () => {
              ...
              try {
                ...
                if (typeof logActivity === "function") {
                  await logActivity({
                    type: "Cart",
                    message: `Cart '${pendingCartName}' created in invoice #${
          ...
          <CartMergeModal
            ...
            onMerge={async (sourceId: string, targetId: string) => {
              try {
                ...
                if (typeof logActivity === "function") {
                  const sourceCart = cartsToMerge.find(
                    (c) => c.id === sourceId
                  );
                  const targetCart = cartsToMerge.find(
                    (c) => c.id === targetId
                  );
                  await logActivity({
                    type: "Cart",
                    message: `Merged cart "${sourceCart?.name}" into "${
                      targetCart?.name
                    }" in invoice #${
                      localInvoice.invoiceNumber || localInvoice.id
                    }`,
        ...
        {showCartPrintModal &&
          (() => {
            const cart = localInvoice.carts.find(
              (c) => c.id === showCartPrintModal.cartId
            );
            if (!cart) return null;

            // Get client print configuration with defaults (same as Print All Carts)
            const printConfig: PrintConfiguration["cartPrintSettings"] =
              client?.printConfig?.cartPrintSettings ||
              ({
                enabled: true,
                showProductDetails: true,
                showProductSummary: false,
                  ...
                  <div className="modal-content" style={{ height: "90vh" }}>
                    ...
                    >
                      <div className="h-100">
                        ...
                        <div className="h-100">
                          <h6 className="mb-3">
                            📄 Print Preview (8" x 5" - Same as Print All Carts)
                          </h6>
                          <div
                            id="cart-print-area"
                            style={{
                            ...
                            dangerouslySetInnerHTML={{
                              __html: generateSingleCartContent(),
                            }}
                          />
                        </div>
                      </div>
                ...
                <div className="modal-body" id="print-checklist-content">
                  <div style={{ textAlign: "center", marginBottom: 20 }}>
                    ...
                    <div style={{ fontSize: 12, color: "#666" }}>
                      ...
                      {localInvoice.deliveryDate &&
                        ` | Delivery: ${formatDateOnlySpanish(
                          localInvoice.deliveryDate
                        )}`}
                    </div>
                  </div>
                ...
                <div className="modal-footer d-print-none">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (!printWindow) {
                        alert(
                          "Please allow popups to print the loading checklist"
                        );
                        return;
                      }
                      ...
                      <html>
                        <head>
                          <title>Loading Checklist - ${transformClientNameForDisplay(
                            localInvoice.clientName
                          )}</title>
                          <style>
                            @page { 
                        ...
                        <body>
                          <div class="compact-layout">
                            <h1>${transformClientNameForDisplay(
                              localInvoice.clientName
                            )}</h1>
        ...
        {showSpecialServiceConfirmation && (
          <DeleteConfirmationModal
            ...
            message={`Are you sure you want to ${
              pendingSpecialServiceState ? "enable" : "disable"
            } special service for this invoice?`}
            confirmButtonText={
              pendingSpecialServiceState
                ? "Enable Special Service"
                : "Disable Special Service"
            }
            confirmButtonClass={
              pendingSpecialServiceState ? "btn-primary" : "btn-warning"
            }
          />
        )}
      </div>
                            ...
                            const addProductCallback = async () => {
                              ...
                              setLocalCarts((prevCarts) =>
                                prevCarts.map((cartObj) => {
                                  if (cartObj.id !== showProductKeypad.cartId)
                                    return cartObj;
