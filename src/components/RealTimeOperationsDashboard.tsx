...
const RealTimeOperationsDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeProduction = productionService.subscribe(
      (summary: ProductionSummary) => {
        console.log(
          "🏭 [Operations Dashboard] Received production update:",
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const todayInvoices: Invoice[] = snapshot.docs
        .map((doc) => {
          const data = doc.data();
          return {
            id: doc.id,
  ...
  useEffect(() => {
    ...
    const totalPoundsEntered = pickupEntries.reduce(
      (sum, entry) => sum + entry.weight,
      0
    );

    // Total pounds washed in Tunnel today (groups with Tunnel status and washed=true)
    const tunnelGroupsWashed = pickupGroups.filter(
      (group) =>
        (group.status === "Tunnel" || group.washingType === "Tunnel") &&
        group.washed
    );
    const totalPoundsWashedTunnel = tunnelGroupsWashed.reduce(
    ...
    const totalItemsProcessed = invoices.reduce((sum, invoice) => {
      return (
        sum +
        invoice.carts.reduce((cartSum, cart) => {
          return (
            cartSum +
            (cart.items || []).reduce(
            ...
            <div className="card-body text-center">
              ...
              <p className="text-muted mb-2">
                ...
                {productionSummary && productionSummary.currentHourRate > 0 && (
                  <span>
                    {" "}
                    • {Math.round(productionSummary.currentHourRate)}/hr current
                    rate
                  </span>
                )}
              </p>
        ...
        <div className="col-md-3 mb-3">
          <div className="card border-success h-100">
            <div className="card-body text-center">
              ...
              <p className="text-muted mb-2">
                From {formatNumber(metrics.totalTunnelGroupsWashed)} tunnel
                groups completed
              </p>
              <small className="text-muted">
                Last updated: {metrics.lastUpdate.toLocaleTimeString()}
                ...
                <div className="col-6">
                  <div className="border-end">
                    <h4 className="text-primary">
                      {formatNumber(metrics.totalPickupEntries)}
                    </h4>
              ...
              <div className="d-flex justify-content-between">
                ...
                <strong>
                  {metrics.totalPickupEntries > 0
                    ? formatWeight(
                        metrics.totalPoundsEntered / metrics.totalPickupEntries
                      )
                    : "0 lbs"}
                </strong>
              </div>
                ...
                <div className="col-6">
                  <div className="border-end">
                    <h4 className="text-success">
                      {formatNumber(metrics.totalTunnelGroupsWashed)}
                    </h4>
              ...
              <div className="d-flex justify-content-between">
                ...
                <strong>
                  {metrics.totalTunnelGroupsWashed > 0
                    ? formatWeight(
                        metrics.totalPoundsWashedTunnel /
                          metrics.totalTunnelGroupsWashed
                      )
                    : "0 lbs"}
                </strong>
              </div>
              ...
              {tunnelGroups.length === 0 ? (
                <div className="text-center text-muted py-4">
                  ...
                  <p className="mt-2 mb-0">
                    No tunnel groups in progress today
                  </p>
                </div>
              ) : (
                <div className="row">
                      ...
                      <div className="fw-bold text-info">
                        {Math.round(
                          tunnelGroups.reduce(
                            ...
                            0
                          ) / Math.max(tunnelGroups.length, 1)
                        )}
                        %
                      </div>
          ...
          <div className="col-12">
            <div className="card">
              <div className="card-header">
                <h5 className="mb-0 d-flex align-items-center justify-content-between">
                  <span>
                    🏭 Real-Time Production Rates (Items Added to Invoices)
                  </span>
                ...
                {productionSummary.topProductsByRate.length === 0 ? (
                  ...
                  <>
                    ...
                    <div className="mb-4">
                      ...
                      <div className="row">
                        {productionSummary.topProductsByRate
                          .slice(0, 8)
                          .map((product, index) => {
                    ...
                    <div className="border-top pt-3 mt-3">
                      <div className="row text-center">
                        ...
                        <div className="col-md-3">
                          <div className="fw-bold text-info">
                            {Math.round(
                              productionSummary.topProductsByRate.reduce(
                                (sum, p) => sum + p.ratePerHour,
                                0
                              ) /
                                Math.max(
                                  productionSummary.topProductsByRate.length,
                                  1
                                )
                            )}
                          </div>
      ...
      <div className="row mt-4">
        <div className="col-12">
          <div className="alert alert-info text-center">
            <small>
              🔄 This dashboard updates automatically in real-time as data is
              added to the system. Last update:{" "}
              {metrics.lastUpdate.toLocaleString()}
            </small>
          </div>
        </div>
      </div>
