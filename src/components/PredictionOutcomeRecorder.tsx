...
const PredictionOutcomeRecorder: React.FC = () => {
  ...
  const recordActualOutcome = async (
    ...
    try {
      ...
      console.log(
        `✅ Recorded outcome for ${record.date}: ${actualWeight} lbs (predicted: ${record.predictedWeight} lbs)`
      );
    } catch (error) {
  ...
  const getAccuracyPercentage = (predicted: number, actual?: number) => {
    ...
    return (
      (100 - Math.abs((predicted - actual) / predicted) * 100).toFixed(1) + "%"
    );
  };
        ...
        <div className="card-body text-center">
          ...
          <div className="row">
            ...
            <div className="col-md-6 mb-2">
              ...
              <small className="text-muted d-block mt-1">
                Learn from historical pickup data
              </small>
            </div>
          </div>
        </div>
      ...
      <div className="card border-success">
        ...
        <div className="card-body">
          <div className="alert alert-success">
            ...
            <strong>How it works:</strong> The system automatically analyzes
            your historical pickup data and compares it with predictions to
            train the ML models. No manual data entry required!
          </div>
              ...
              <div className="col-md-6">
                <div className="card bg-light">
                  <div className="card-body">
                    ...
                    <div className="mb-2">
                      <strong>Total Comparisons:</strong>{" "}
                      {autoLearningStats.totalComparisons}
                    </div>
                    <div className="mb-2">
                      <strong>Average Accuracy:</strong>{" "}
                      <span
                        className={`badge ${
                          autoLearningStats.averageAccuracy >= 75
                            ? "bg-success"
                            : autoLearningStats.averageAccuracy >= 60
                            ? "bg-warning"
                            : "bg-danger"
                        }`}
                      >
                    ...
                    <div className="mb-2">
                      ...
                      <span
                        className={`badge ${
                          autoLearningStats.recentAccuracy >= 75
                            ? "bg-success"
                            : autoLearningStats.recentAccuracy >= 60
                            ? "bg-warning"
                            : "bg-danger"
                        }`}
                      >
                    ...
                    <div>
                      ...
                      <span
                        className={`badge ${
                          autoLearningStats.improvementTrend === "Improving"
                            ? "bg-success"
                            : autoLearningStats.improvementTrend === "Declining"
                            ? "bg-warning"
                            : "bg-secondary"
                        }`}
                      >
              ...
              <div className="col-md-6">
                <div className="card bg-light">
                  <div className="card-body">
                    ...
                    {Object.entries(autoLearningStats.accuracyByDayOfWeek).map(
                      ([day, accuracy]) => {
                        const dayNames = [
                          "Sunday",
                          "Monday",
    ...
    <div className="card border-primary">
      <div className="card-header bg-primary text-white d-flex justify-content-between align-items-center">
        <div>
          ...
          <small>
            Help the system learn by recording actual vs predicted weights
          </small>
        </div>
        <button
          className="btn btn-outline-light btn-sm"
        ...
        <div className="row mb-3">
          <div className="col-12">
            <div className="alert alert-info">
              <i className="bi bi-lightbulb me-2"></i>
              <strong>Manual Entry:</strong> Enter the actual weight collected
              for each day. The ML system will learn from the difference between
              predicted and actual values. After 10+ recordings, automatic
              retraining begins.
              <br />
              <small className="mt-2 d-block">
                💡 <strong>Tip:</strong> You can also use{" "}
                <button
                  className="btn btn-link btn-sm p-0"
                  onClick={() => {
                    setShowAutoLearning(true);
                ...
                <tr key={record.date}>
                  ...
                  <td>
                    <span className="badge bg-info">
                      {record.predictedWeight} lbs
                    </span>
                  </td>
                  <td>
                    {record.status === "recorded" ? (
                      <span className="badge bg-success">
                        {record.actualWeight} lbs
                      </span>
                    ) : (
                      <ActualWeightInput
                        onSubmit={(weight) =>
                          recordActualOutcome(record, weight)
                        }
                        loading={loading}
                      />
                    )}
            ...
            <div className="card bg-light">
              <div className="card-body">
                ...
                <small className="text-muted">
                  {
                    pendingPredictions.filter((p) => p.status === "recorded")
                      .length
                  }
                  /10 outcomes needed for retraining
                </small>
              </div>
            </div>
          </div>
                ...
                <div className="d-flex align-items-center">
                  <div className="me-3">
                    <span className="badge bg-primary fs-6">
                      {(() => {
                        const recorded = pendingPredictions.filter(
                          (p) => p.status === "recorded"
                        );
                        if (recorded.length === 0) return "N/A";
                        const avgAccuracy =
                          recorded.reduce((sum, r) => {
                            const acc =
                              100 -
                              Math.abs(
                                (r.predictedWeight - (r.actualWeight || 0)) /
                                  r.predictedWeight
                              ) *
                                100;
                            return sum + acc;
...
const ActualWeightInput: React.FC<{
  ...
  return (
    <div className="input-group input-group-sm" style={{ minWidth: "120px" }}>
      ...
      <button className="btn btn-secondary" onClick={() => setShowInput(false)}>
